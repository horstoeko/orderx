    ...
    if (!empty($commits)) {
        ...
        foreach ($commits as $commit) {
            ...
            $commitIssuesWithUrls = array_map(function ($issue) {
                return sprintf('[%1$s](https://github.com/horstoeko/orderx/issues/%2$s)', $issue, substr($issue, 1));
            }, $commitIssues);
            ...
            $markDown[] = sprintf('| %6$s | [%1$s](https://github.com/horstoeko/orderx/commit/%1$s) | %2$s | %3$s | %4$s | %5$s', $commitHash, $commitDate, $commitAuthor, $commitSubject, implode(", ", $commitIssuesWithUrls), $commitSubjectIcons);
        }
